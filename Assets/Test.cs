...
public class Test :MonoBehaviour {
	...
	public UISprite[] sps;
	public UIGrid grid;
	public void SetState(int x,int y,int masuXY,Color col){
		sps[x*masuXY+y].color=col;
	}
	...
	void UpdateGrids(){
		for(int r=0;r<PSParams.GameParameters.mapMasuXY;r++){
			for(int v=0;v<PSParams.GameParameters.mapMasuXY;v++){
				if(isSafeZone(r,v)){
					SetState(r,v,PSParams.GameParameters.mapMasuXY,Color.white);
				}else{
					SetState(r,v,PSParams.GameParameters.mapMasuXY,Color.red);
				}
			}
		}
	...
	void Awake(){
		sps=new UISprite[ grid.GetChildList().Count];
		int i=0;
		foreach(Transform tr in grid.GetChildList()){
			sps[i]=tr.gameObject.GetComponent<UISprite>();
			i++;
		}
	}
	...
	void Start(){
		InitSafeZone(PSParams.GameParameters.mapMasuXY,new Vector3(-100.0f,0.0f,100.0f),new Vector3(100.0f,0.0f,-100.0f));
		UpdateGrids();
	}
	...
	public void TestInvoke(){
		IntVector2 kouho=GetNextDangerZoneRandom();
		if(kouho!=null){
			SetSafeZone(kouho.x,kouho.y,false);
			UpdateGrids();

		}else{
			Debug.LogError("全部埋まった");
		}
	}
	...
	public bool IsReachable(IntVector2 kouho){
		...
		foreach(int i in reachedPoints){
			sps[i].color=Color.blue;
		}
