...
public class PickupAndWeaponManager : PS_SingletonBehaviour<PickupAndWeaponManager> {
	...
	public Transform SpawnSubweapon_Yudoudan(shipControl ship,Vector3 position,Quaternion qt,float spawnTime){
		Transform trans=Spawn(subweaponYudoudan_Prefab,position,qt,null);
		trans.gameObject.GetComponent<SubweaponShot>().Spawn(ship,spawnTime,position);
		return trans;
	...
	public Transform SpawnSubweapon_Nuke(shipControl ship,Vector3 position,Quaternion qt,float spawnTime){
	...
	public Transform SpawnSubweapon_Napam(shipControl ship,Vector3 position,Quaternion qt,float spawnTime){
	...
	public Transform SpawnSubweapon_Zenhoukou(shipControl ship,Vector3 position,Quaternion qt,float spawnTime){
	...
	public Transform SpawnSubweapon_Wave(shipControl ship,Vector3 position,Quaternion qt,float spawnTime){
		Transform trans=Spawn(subweaponWave_Prefab,position,qt,null);
		if(ship)trans.gameObject.GetComponent<SubweaponShot>().Spawn(ship,spawnTime,position);
		return trans;
	...
	public Transform SpawnShot(shipControl ship,DanmakuColor col,Vector3 position,Quaternion qt,float spawnTime){

		Transform trans=Spawn(danmaku[(int)col],position,qt,null);
		if(ship)trans.gameObject.GetComponent<shot>().Spawn(ship,spawnTime,position);
		return trans;
	}
